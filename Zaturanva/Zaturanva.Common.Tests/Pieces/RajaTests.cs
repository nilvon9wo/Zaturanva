...
public class RajaTests
{
	[Theory]
	[InlineData("A1", "B2", true)] // Valid move to the top-right
	[InlineData("D4", "C5", true)] // Valid move to the top-right
	[InlineData("E4", "D5", true)] // Valid move to the bottom-left
	[InlineData("D5", "E4", true)] // Valid move to the top-left
	[InlineData("G7", "F6", true)] // Valid move to the bottom-right
	[InlineData("B2", "A1", true)] // Too close in the top-left direction
	...
	)
	{
		...
		Raja raja = CreateRajaAt(player, initialPosition);
		GameState game = CreateGameFor(raja);

		// Act
		bool canMove = raja.CanMoveTo(game, destinationPosition);
	...
	)
	{
		...
		// Act
		bool canMove = raja.CanMoveTo(game, new(destinationX, destinationY));
	...
	{
		Players players = new() { raja.Owner };
		IEnumerable<IPiece> allPieces = new List<IPiece> { raja };
